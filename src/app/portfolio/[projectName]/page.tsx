...
const Page = ({ params }: { params: any }) => {
	...
	return (
		<>
			...
			{ListOfImages.length > 0 && <Gallery project={pageProject} />}
		</>
	);
};
...
const Header = ({ project }: { project: any }) => {
	...
	return (
		<>
			<Image src={`/image/stormtrooper.jpg`} alt='Hero BG' className='fixed h-full object-cover' width={2900} height={1200} priority />
			...
			<FullSection id='Project'>
				<header className='z-20 mb-20 grid grid-cols-5 gap-6 xl:mb-0 xl:gap-20'>
					...
					<div className='content radial_contrast relative col-span-5 flex flex-col justify-center xl:col-span-3'>
						...
						<p className='description text-ms lg:text-base'>{description}</p>
						<div className='my-4 flex'>
							{url && (
								<Link href={url} target='_blank' className='mr-4 flex transition-all ease-in hover:font-bold xl:hover:scale-105'>
									Ir al sitio <Icon.ExternalLink className='ml-2' size={24} />
								</Link>
							)}
							{repo && (
								<Link href={repo} target='_blank' className='mr-4 flex transition-all ease-in hover:font-bold xl:hover:scale-105'>
									Repo <Icon.ExternalLink className='ml-2' size={24} />
								</Link>
							)}
						</div>
