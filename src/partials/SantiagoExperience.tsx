...
const ExperienceResume = () => {
  return (
    <div className="santiago_content col-span-3 mb-12 xl:mb-0">
      <h1 className="title text-5xl font-bold mb-6">
        Something about my work experience
      </h1>
      <p>
        <span>
          As a <span className="text-rose-900 font-bold">passionate FrontEnd developer</span>, I have specialized in <span className="text-rose-900 font-bold">React.js</span>, creating <span className="text-rose-900 font-bold">exceptional web experiences</span> with JavaScript and CSS. At <span className="text-rose-900 font-bold">Movidagráfica™</span>, I had the opportunity to lead a small team on a high-demand project, allowing me to demonstrate my leadership skills and attention to detail.
        </span>
        <br/>
        <br/>
        <span>
          My experience also includes working as a <span className="text-rose-900 font-bold">FrontEnd Developer</span> at <span className="text-rose-900 font-bold">Studio Salvatori</span>, where I learned the fundamentals of web development. My technical skills and experiences include: <span className="text-rose-900 font-bold">React, NextJS, HTML, CSS and SASS, JavaScript and TypeScript, Git and GitHub, WordPress and ACF, Laravel, BootstrapCSS, BulmaCSS, and TailwindCSS</span>.
        </span>
        <br/>
        <br/>
        <span>
          In addition to my technical experience, I hold a <Link href={'https://www.efset.org/cert/xFaeXR'} className="text-rose-900 font-bold">B2 level English certification</Link>, allowing me to communicate effectively in a global environment. I am excited to contribute to software development with innovative solutions, staying updated with the latest technological trends. I am ready to contribute with passion and commitment to a team where I can continue to grow professionally!
        </span>
      </p>
    </div>
  )
...
const Experience = ({ date, agencie, position }: { date: string, agencie: string, position: string }) => {
  return (
    <div className="relative w-full md:w-2/4 xl:w-full flex items-center justify-between xl:justify-normal xl:even:flex-row-reverse xl:even:text-right group is-active">
      <div className="flex items-center justify-center w-10 h-10 rounded-full border group-[.is-active]:bg-cyan-400 text-slate-500 group-[.is-active]:text-emerald-50 shadow shrink-0 xl:order-1 xl:group-odd:-translate-x-1/2 xl:group-even:translate-x-1/2">
          <Icon.Check size={21} />
      </div>
      <div className="w-[calc(100%-4rem)] xl:w-[calc(50%-2.5rem)] bg-gradient-to-tl from-rose-500 to-cyan-400 p-4 rounded border shadow-lg transition-all duration-300">
          <time className="font-caveat font-medium text-xs">
            {date}
          </time>
          <div className="font-bold text-ms text-rose-800">
            {agencie}
          </div>
